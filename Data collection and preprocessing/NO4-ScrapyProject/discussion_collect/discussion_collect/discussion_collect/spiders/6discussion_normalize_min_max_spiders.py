...
class Discussionspider(Spider):
    ...
    def start_requests(self):
        ...
        self.logger.info("===== 开始执行：仅归一化（读取噪声处理后文件，不重复噪声处理） =====")
    ...
    def load_noise_processed_data(self):
        ...
        try:
            ...
            for field in self.numeric_fields:
                ...
                self.logger.info(f"  • {field}：有效数值数={valid_count} 条，无效值数={invalid_count} 条")
    ...
    def process_min_max_normalization(self, df):
        ...
        for field in self.numeric_fields:
            ...
            if len(valid_series) == 0:
                self.logger.info(f"  无有效数值，跳过归一化")
                continue
            ...
            self.logger.info(f"  噪声处理后原始范围：[{original_min:.2f}, {original_max:.2f}]")
            
            # 步骤3：处理极值相同的情况（避免除零错误）
            if original_max == original_min:
                self.logger.info(f"  所有有效值均为 {original_min}，归一化为目标区间最小值 {target_min}")
            ...
            self.logger.info(f"  归一化完成，目标区间：[{target_min}, {target_max}]")
            self.logger.info(f"  归一化后均值：{normalize_stats[field]['mean_after_norm']:.6f}")
    ...
    def print_normalize_stats(self, normalize_stats):
        """打印详细的归一化统计报告"""
        self.logger.info("\n" + "="*60)
        self.logger.info("===== 归一化统计报告 =====")
        self.logger.info("="*60)
        
        for field, stats in normalize_stats.items():
            self.logger.info(f"\n【{field}】")
            if stats['original_min'] is None:
                self.logger.info(f"  状态：未执行归一化（无有效数值）")
            else:
                self.logger.info(f"  噪声处理后原始范围：[{stats['original_min']:.2f}, {stats['original_max']:.2f}]")
                self.logger.info(f"  归一化目标范围：[{stats['normalized_min']}, {stats['normalized_max']}]")
                self.logger.info(f"  归一化后均值：{stats['mean_after_norm']:.6f}")
            self.logger.info(f"  有效数值数：{stats['valid_count']} 条")
            self.logger.info(f"  无效数值数：{stats['invalid_count']} 条")

    def print_final_data(self, df_final):
        """打印归一化后的最终数据（概览+前N条详情）"""
        self.logger.info("\n" + "="*60)
        self.logger.info(f"===== 归一化后最终数据结果（共 {len(df_final)} 条） =====")
        self.logger.info("="*60)
        
        # 1. 数据基本信息
        self.logger.info(f"\n【数据基本信息】")
        self.logger.info(f"总记录数：{len(df_final)}")
        self.logger.info(f"字段列表：{list(df_final.columns)}")
        ...
        for field in self.numeric_fields:
            ...
            if len(valid_series) > 0:
                ...
                self.logger.info(f"    - 均值：{valid_series.mean():.6f}")
                self.logger.info(f"    - 最小值：{valid_series.min():.6f}")
                self.logger.info(f"    - 最大值：{valid_series.max():.6f}")
                self.logger.info(f"    - 标准差：{valid_series.std():.6f}")
                self.logger.info(f"    - 中位数：{valid_series.median():.6f}")
            else:
                self.logger.info(f"  • {field}：无有效归一化数值")
