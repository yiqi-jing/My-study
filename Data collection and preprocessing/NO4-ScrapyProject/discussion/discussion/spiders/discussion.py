...
class DiscussionSpider(Spider):
    ...
    # 分文件夹存储：CSV和头像分开
    csv_dir = 'discussion/csv'          # CSV文件文件夹
    avatar_dir = 'discussion/avatars'   # 头像文件文件夹
    ...
    def update_avatar_progress(self, is_success):
        ...
        with self.progress_lock:
            ...
            if data_finished and avatar_finished:
                ...
                print(f'🖼️  头像进度：{self.generate_progress_bar(self.successful_avatar_downloads, self.total_avatar_tasks)}\n', flush=True)
                return
            ...
            print(f'🖼️  头像进度：{avatar_bar}', end='\r', flush=True)
    ...
    def download_avatar(self, avatar_url, author_name, topic_index):
        ...
        try:
            ...
            self.logger.warning(f'⚠️  头像下载失败 | 作者：{author_name} | 错误：{str(e)[:100]}')
            self.update_avatar_progress(False)
            return None
    ...
    def parse_topic_list(self, response):
        ...
        try:
            ...
            for row_idx, row in enumerate(topic_rows):
                ...
                # 提交头像下载任务到线程池
                current_topic_index = len(self.all_data) + 1
    ...
    def handle_error(self, failure):
        ...
        self.logger.error(f'❌ 请求失败：{request.url} | 错误类型：{str(failure.type)[:30]}', exc_info=False)
        if len(self.all_data) < self.target_count:
            self.current_page += 1
            next_page_url = self.url_template.format(self.current_page)
            yield Request(
    ...
    def closed(self, reason):
        ...
        self.logger.warning(f'🖼️  最终头像统计：')
